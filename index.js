...
*/
const pizza = MenuElemaniOlustur("Karışık Pizza", 5, "Pizzalar");
const hamburger = MenuElemaniOlustur("Cheeseburger", 8, "Burgerler");
const sushi = MenuElemaniOlustur("Sushi", 7, "Asya");

// Oluşturulan menü elemanlarını konsolda görüntüleyin
console.log(pizza);
...
function AnahtardanDegerlendirmeAl(degerlendirmeler, indeks) {
  ...
  const cikti = `${degerlendirme.isim} isimli kişi ${degerlendirme.puan} puan verdi ve şunları yazdı: ${degerlendirme.geribildirim}`;
  return cikti;
}
...
function SonDegerlendirmeyiAl(degerlendirmeler) {
  ...
  const cikti = `${sonDegerlendirme.isim} isimli kişi ${sonDegerlendirme.puan} puan verdi ve şunları yazdı: ${sonDegerlendirme.geribildirim}`;
  return cikti;
}
